...
用法:
    from src.detector.overlay import show_toast, extract_key_name
...
class ToastOverlay:
    ...
    def show(self, key_name: str, parent_hwnd: int):
        ...
        print(f"[Toast] show() called | key_name={key_name!r} | parent_hwnd={parent_hwnd}",
              flush=True)
        ...
        if parent_hwnd and user32.IsWindow(parent_hwnd):
            ...
            print(f"[Toast] Parent window: left={rect.left} top={rect.top} "
            ...
            print(f"[Toast] No valid parent window, fallback to screen: {rect.right}x{rect.bottom}",
                  flush=True)
        ...
        print(f"[Toast] Tk window HWND = {hwnd} (0x{hwnd:08X})", flush=True)
        ...
        print(f"[Toast] Window geo: {toast_w}x{toast_h}+{toast_x}+{toast_y}",
              flush=True)
        ...
        # -- 立即渲染 --
        self._win.update()
        print(f"[Toast] Window created and rendered, will disappear after {self.DURATION}s", flush=True)

    # ------------------------------------------------------------------
    def _on_timeout(self):
        """定时器回调：销毁窗口。"""
        print("[Toast] Timer triggered → destroy window", flush=True)
        self.destroy()
...
def show_toast(key_name: str, parent_hwnd: int):
    """显示 Toast 覆盖层。"""
    global _toast
    print(f"[Toast] show_toast() entry | key_name={key_name!r} | hwnd={parent_hwnd}",
          flush=True)
    if _toast is None:
        _toast = ToastOverlay()
