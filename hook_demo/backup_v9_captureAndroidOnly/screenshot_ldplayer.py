...
SEARCH_EXPAND = 16         # 向左、向上额外延长（参考像素）
...
# ---------------------------------------------------------------------------
# 4b. 获取 dnplayer 内部 RenderWindow 子窗口屏幕坐标（纯安卓画面）
# ---------------------------------------------------------------------------
def get_dnplayer_render_rect():
    """返回 dnplayer 内 RenderWindow 子窗口的屏幕坐标。
...
# ---------------------------------------------------------------------------
# 4c. 获取 dnplayer.exe 主窗口句柄（用于 Toast 定位）
# ---------------------------------------------------------------------------
...
def resolve_capture_target(process_name: str):
    ...
    if process_name.lower() == "ld9boxheadless.exe":
        # 优先：RenderWindow 子窗口（纯安卓画面，无上边栏和右侧工具栏）
        render_rect = get_dnplayer_render_rect()
        if render_rect:
            return render_rect, "dnplayer RenderWindow (纯安卓画面)", []

        # 回退：dnplayer 客户端区域（含上边栏+右侧工具栏）
...
def match_sample_in_corner(screenshot_bgr, sample_path: str, scale: float,
                           right_pad: int, bottom_pad: int, right_trim: int):
    """
    ...
    参数：
        ...
        scale: 实际游戏区高度 / 参考游戏区高度
        right_pad:  参考像素下，搜索区域右边距（图标距截图右边 + slack + expand）
        bottom_pad: 参考像素下，搜索区域下边距
        right_trim: 参考像素下，搜索区域右侧缩进
    ...
    region_r = w - int(right_trim * scale)
...
# ---------------------------------------------------------------------------
def match_drive_walk(screenshot_bgr, quiet=False, capture_source=""):
    """
    ...
    if is_game_area_only:
        # 纯安卓画面截图：无上边栏/右侧工具栏，参考 = 游戏区 1920x1080
        ref_h = REF_GAME_H          # 1080
        ref_w = REF_GAME_W          # 1920
        right_pad = REF_MARGIN_RIGHT + SEARCH_SLACK + SEARCH_EXPAND    # 40+20+16=76
        bottom_pad = REF_MARGIN_BOTTOM + SEARCH_SLACK + SEARCH_EXPAND  # 40+20+16=76
        right_trim = 0
        extra_info = f"纯游戏区 {ref_w}x{ref_h}"
    else:
        # 含标题栏+工具栏截图：参考 = 1980x1140
        ref_h = REF_CAPTURE_H       # 1140
        ref_w = REF_CAPTURE_W       # 1980
        right_pad = REF_ICON_RIGHT_PAD + SEARCH_SLACK + SEARCH_EXPAND        # 100+20+16=136
        bottom_pad = REF_ICON_BOTTOM_PAD + SEARCH_SLACK + REF_BOTTOM_EXTRA + SEARCH_EXPAND  # 40+20+13+16=89
        right_trim = REF_RIGHT_TRIM
        extra_info = f"游戏区 {REF_GAME_W}x{REF_GAME_H} + 上栏{REF_TITLEBAR_H} + 工具栏{REF_TOOLBAR_W}"

    scale = h / ref_h
    if not quiet:
        print(f"[比例] 截图 {w}x{h}，参考 {ref_w}x{ref_h}，scale={scale:.4f}")
        print(f"       ({extra_info})")
    ...
    for name, path in (("drive", DRIVE_SAMPLE), ("walk", WALK_SAMPLE)):
        ...
        val, loc, region_info, size = match_sample_in_corner(
            screenshot_bgr, path, scale, right_pad, bottom_pad, right_trim
        )
...
def run_monitor_loop(no_debug=False, parent_hwnd=None):
    ...
    try:
        while True:
            ...
            # 检测截图来源变化（首次使用 RenderWindow 时通知）
            if source != last_source and last_source is not None:
                print(f"  [截图来源切换] \"{last_source}\" → \"{source}\"")
            ...
            results = match_drive_walk(screenshot_bgr, quiet=True, capture_source=source)
            ...
            if best_name is None:
                ...
                if os.path.exists(kmp):
                    ...
                    print(f"[切换] 显示 Toast: kmp={os.path.basename(kmp)} → key_name={key_name!r}",
                          flush=True)
...
def main():
    ...
    # ---- 获取 dnplayer 主窗口句柄（用于 Toast 定位）----
    dn_hwnd = get_dnplayer_hwnd()
    if dn_hwnd:
        print(f"[窗口] dnplayer HWND = {dn_hwnd}")
    else:
        print("[警告] 未找到 dnplayer 主窗口，Toast 将居中屏幕显示。")

    # ---- 进入循环监控 ----
    run_monitor_loop(no_debug=args.no_debug, parent_hwnd=dn_hwnd)
