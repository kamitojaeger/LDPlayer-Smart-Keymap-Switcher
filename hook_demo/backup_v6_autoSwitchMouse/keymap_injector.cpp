...
static void SendCtrlF(HWND hwnd) {
    // PostMessage to LDPlayer window (doesn't need foreground, bypasses LL hook filter)
    if (hwnd) {
        ...
        Sleep(50);  // brief settle after dispatch
    }
    // Note: SendInput backup removed — PostMessage is reliable and SendInput is
    // filtered by LDPlayer's WH_KEYBOARD_LL hook (LLKHF_INJECTED flag).
}
...
static bool TriggerOnceAndVerify(HWND hwnd, const wchar_t* targetKmp) {
    ...
    SendCtrlF(hwnd);
    Sleep(200);  // hook fires synchronously; 200ms enough for .kmp read + setKeyboardConfig
...
int wmain(int argc, wchar_t* argv[]) {
    ...
    if (mode == Mode::Init) {
        ...
        if (OpenSharedMem()) {
            ...
            Sleep(500);  // brief wait for DllMain hooks to stabilize
        }
        SharedData* p = OpenSharedMem();
