...
**功能：** 提前注入 DLL 并等待初始化完成（约2秒）

**用途：** 首次执行切换命令时需要注入 DLL + 等待2秒初始化。使用 init 提前完成此步骤，之后切换命令无需等待，执行更快。

**推荐流程：**
```
# 1. 启动 LDPlayer 后，先初始化（一次性）
keymap_injector.exe init
...
**功能：** 注入 DLL（如未初始化）+ 触发一次 Ctrl+F + CFW 重定向
...
- v3: 多版本兼容（LDPlayer9 + LDPlayer14，已备份在 backup_v3_multi/）
  - 自动检测2组 dnplycore.dll 偏移
  - CFW hook 改为 kernelbase.CreateFileW（兼容 LDPlayer14）
  - 支持多种进程名
  - 添加版本信息减少杀毒误报
- **v4: 预初始化（当前版本，已备份在 backup_v4_preInit/）**
  - 添加 `init` 命令，提前注入 DLL 避免首次切换的2秒等待
  - 使用流程：先 `init`（一次性），之后切换命令快速执行
